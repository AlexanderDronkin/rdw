...
hydra = {
	...
	editing : function () {
		...
		if ( $('*[name="major_sub_array[20][0]"]').length > 0 ) {
			...
			let remove = [ 'вахта в москве', 'вахта в домодедово', 'работа в москве и московской области' ];
			
			for ( let rs of remove ) {
				
				let exp = new RegExp( `(${rs})`, 'i' );
				if ( desc.match( exp ) ) {
					desc = desc.replace( exp, '' );
					desc_br = desc_br.replace( exp, '<span style="color: #00f137"><b>$1</b></span>' ); 
					let reg_val = $('textarea[name="major_sub_array[8][0]"]').val();
					if ( !reg_val.match( exp ) ) {
						if ( reg_val.length > 0 )
							reg_val += "\n";
						reg_val += "Вахта в Москве";
						$('textarea[name="major_sub_array[8][0]"]').val( reg_val );
					}
				}
			}
